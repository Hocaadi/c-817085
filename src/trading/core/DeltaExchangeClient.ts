...
export class DeltaExchangeClient {
  ...
  async startStrategy(): Promise<boolean> {
    try {
      ...
      toast({
        ...
        description: "Successfully connected to Delta Exchange and activated strategy"
      });
  ...
  async getOrderbook(symbol: string) {
    this.checkStrategyActive();
    const response = await this.makeRequest('GET', `/l2orderbook/${symbol}`);
    return response.result || {};
  }

  async getTrades(symbol: string) {
    this.checkStrategyActive();
    const response = await this.makeRequest('GET', `/trades/${symbol}`);
    return response.result || [];
  }
} 